#include "youbot_arm_moveit_config/arm_utils_v3.h"
...
ArmUtils::ArmUtils(ros::NodeHandle &nh_):                                
        ...
        WHOLE_PLANNING_GROUP("arm_1"),                    // Initialize move_group_name
        ...
        visual_tools("arm_link_0")                      // Initialize MoveItVisualTools
    {
        ...
        // nh_.setParam("moveit_controller_manager", "moveit_simple_controller_manager/MoveItSimpleControllerManager");
        ...
        whole_planning_components_ptr = std::make_shared<moveit_cpp::PlanningComponent>(WHOLE_PLANNING_GROUP, moveit_cpp_ptr);
        whole_joint_model_group_ptr = robot_model_ptr->getJointModelGroup(WHOLE_PLANNING_GROUP);
...
void ArmUtils::goToPose(const Eigen::Vector3d& position, const Eigen::Quaterniond& orientation){
    ...
    whole_planning_components_ptr->setGoal(pose_goals);
    whole_planning_components_ptr->setStartStateToCurrentState();
    planning_interface::MotionPlanResponse sol = whole_planning_components_ptr->plan();
    if (sol.error_code_.val == moveit_msgs::MoveItErrorCodes::SUCCESS) {
        ...
        visual_tools.publishTrajectoryLine(sol.trajectory_, whole_joint_model_group_ptr);
    ...
    if (whole_planning_components_ptr->execute()) {
...
void ArmUtils::aimAtPoint(const Eigen::Vector3d& point){
    ...
    moveit::core::RobotStatePtr current_state = moveit_cpp_ptr->getCurrentState();
    Eigen::Affine3d end_effector_state = current_state->getGlobalLinkTransform("hose_holder_hose_holder_tip_link");
    Eigen::Matrix3d AL0_R_AL4 = end_effector_state.rotation();
    Eigen::Vector3d AL0_p_AL4 = end_effector_state.translation();
    Eigen::Vector3d diff = point - AL0_p_AL4;
    float yaw_rot = atan2(diff.y(), diff.x());
    Eigen::Matrix3d new_rot = Eigen::AngleAxisd(yaw_rot, Eigen::Vector3d::UnitZ()) * AL0_R_AL4;
    Eigen::Vector3d new_pos = Eigen::AngleAxisd(yaw_rot, Eigen::Vector3d::UnitZ()) * AL0_p_AL4;
    diff = point - new_pos;
    float pitch_rot = atan2(diff.z(), diff.x());
    new_rot = Eigen::AngleAxisd(M_PI_2, Eigen::Vector3d::UnitY())*Eigen::AngleAxisd(pitch_rot, Eigen::Vector3d::UnitY()) * new_rot;

    //create goal configuration
    pose.pose.position.x = new_pos.x();
    pose.pose.position.y = new_pos.y();
    pose.pose.position.z = new_pos.z();
    pose.pose.orientation.x = Eigen::Quaterniond(new_rot).x();
    pose.pose.orientation.y = Eigen::Quaterniond(new_rot).y();
    pose.pose.orientation.z = Eigen::Quaterniond(new_rot).z();
    pose.pose.orientation.w = Eigen::Quaterniond(new_rot).w();

    
    visual_tools.publishAxis(pose.pose);
    visual_tools.trigger();
    std::vector<double> tolerance_pose(3, 1e-2);
    std::vector<double> tolerance_angle(3, 1e-2);
    moveit_msgs::Constraints pose_goal = kinematic_constraints::constructGoalConstraints("hose_holder_hose_holder_tip_link", pose, tolerance_pose, tolerance_angle);
    whole_planning_components_ptr->setStartStateToCurrentState();
    std::vector<moveit_msgs::Constraints> pose_goals;
    pose_goals.push_back(pose_goal);
...
void ArmUtils::goToState(const std::string state){
    whole_planning_components_ptr->setGoal(state);
    planning_interface::MotionPlanResponse sol = whole_planning_components_ptr->plan();
    if (sol.error_code_.val == moveit_msgs::MoveItErrorCodes::SUCCESS) {
        ROS_INFO("Successfully found a path to pose.");
    } else {
...
int main(int argc, char** argv) {
    ...
    Eigen::Vector3d new_position = position + Eigen::Vector3d(0.1, 0, 0.0);
    ...
    client.visual_tools.prompt("Press 'next' in the RvizVisualToolsGui window to continue the demo");
    Eigen::Vector3d aim_position = position + Eigen::Vector3d(2, 2, 0);
    aim_position[2] = 0.0;
